...
import { JavBus, JavDB } from "../jav/index";
...
describe("JavParserInfo", () => {
	describe("parseInfo Of Javbus", () => {
		it("should parse info correctly", () => {
			...
			const javbus = new JavBus({} as IRequest);
			...
			expect(result).toMatchObject({
				...
				preview: [
					...Array(20)
						.fill(0)
						.map((_, index) => ({
							raw: `https://pics.dmm.co.jp/digital/video/36doks00615/36doks00615jp-${
								index + 1
							}.jpg`,
							thumbnail: `https://www.javbus.com/pics/sample/aq21_${
								index + 1
							}.jpg`,
						})),
				],
	...
	describe("parseInfo Of JavDB", () => {
		it("should parse info correctly", () => {
			...
			const javdb = new JavDB({} as IRequest);
			const result = javdb.parseInfo(mockHtml);

			expect(result).toMatchObject({
				avNumber: "DOKS-615",
				title:
					"美脚がすぎるOLの誘惑黒パンティストッキング 水端あさみ パンティと生写真付き",
				date: 1725120000000,
				duration: 119,
			});
		});
