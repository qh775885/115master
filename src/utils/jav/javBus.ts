...
import { Jav, JAV_SOURCE, JavInfo } from "./jav";
...
export class JavBus extends Jav {
    ...
    async parseInfoAfter(info: JavInfo): Promise<JavInfo> {
        return {
            ...info,
            title: info.title?.replace(new RegExp(`^${info.avNumber}`, 'i'), '').trim(),
            cover: info.cover ? await imageUrl2BlobUrlOfCors(info.cover, this.url) : '',
        }
    }

    getLabels(dom: Document) {
        const headers = dom.querySelectorAll(`.container .movie .info p > span.header, .container .movie .info p.header`);
    ...
    parseActor() {
        const actors = this.labels['演員']?.parentElement?.nextElementSibling?.nextElementSibling?.querySelectorAll('a');
    ...
    parsePublisher() {
        const publishers = this.labels['發行商']?.parentElement?.querySelectorAll('a');
        return publishers?.length ? Array.from(publishers).map(i => ({
            name: i.textContent!,
            url: i.getAttribute('href') ?? undefined,
        })) : undefined;
    }
    ...
    parseCover(dom: Document) {
        const cover = dom.querySelector('.container .bigImage img')?.getAttribute('src');
    ...
    parsePreview(dom: Document) {
        ...
        return previews.length ? Array.from(previews).map(preview => ({
            ...
            thumbnail: preview.querySelector('img')?.getAttribute('src') ?? undefined,
        }))
            .filter(i => !!i.raw || !!i.thumbnail)
            .map(i => {
                return {
                    raw: i.raw,
                    thumbnail: i.thumbnail ? new URL(i.thumbnail, this.baseUrl).href : undefined,
                }
            }) : undefined;
    }
    ...
    parseCategory() {
        const categories = this.labels['類別']?.nextElementSibling?.querySelectorAll('a');
