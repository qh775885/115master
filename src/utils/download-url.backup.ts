...
class Drive115 {
    ...
    private async getDownloadUrlByNormal(pickcode: string): Promise<DownloadResult> {
        ...
        console.log('普通方式获取响应:', response.data);

        if (!response.data.state || !response.data.file_url) {
            throw new Error('服务器返回数据格式错误: ' + JSON.stringify(response.data));
        }
    ...
    private async getDownloadUrlByPro(pickcode: string): Promise<DownloadResult> {
        ...
        const response = await axios.post(
            `https://proapi.115.com/app/chrome/downurl?t=${tm}`,
            data,
            { headers }
        );

        console.log('set-cookie', response.headers['set-cookie'])
        const fileToken = response.headers['set-cookie']?.[1]?.split('; ')?.[0];

        console.log('Pro方式响应:', response.data);

        if (!response.data.state) {
            throw new Error('获取下载地址失败: ' + JSON.stringify(response.data));
        }

        const result = JSON.parse(this.crypto115.m115_decode(response.data.data, encoded.key));
