...
export type CacheMetaItem = {
	key: string; // 原始缓存键
	fullKey: string; // 完整键（包含 name 和 storeName）
	storeName: string; // 存储实例名称
...
export class MetaStore {
	 ...
	 * @param key 缓存键
	 */
	public async getMeta(key: string): Promise<CacheMetaItem | null> {
		const fullKey = this.generateFullKey(key);
	 ...
	 * @param key 缓存键
	 */
	public async removeMeta(key: string): Promise<void> {
		const fullKey = this.generateFullKey(key);
	 ...
	 * @returns 所有缓存项的元数据数组
	 */
	public async getAllMeta(): Promise<CacheMetaItem[]> {
	 ...
	 */
	public async getSortedByLastAccessed(
		ascending = true,
	 ...
	 * @returns 排序后的缓存项元数据数组
	 */
	public async getSortedByCreatedAt(
		ascending = true,
	): Promise<CacheMetaItem[]> {
		const items = await this.getAllMeta();
	 ...
	 * @returns 排序后的缓存项元数据数组
	 */
	public async getSortedByUpdatedAt(
		ascending = true,
	): Promise<CacheMetaItem[]> {
		const items = await this.getAllMeta();
