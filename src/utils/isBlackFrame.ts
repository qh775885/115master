 ...
 * 获取图片中心区域的范围
 */
const getCenterRegion = (width: number, height: number) => {
 ...
 */
export const isBlackFrameFromBase64 = (
	base64: string,
	options: BlackFrameOptions = {},
): Promise<boolean> => {
 ...
 * 使用示例：
 *
 * import { isBlackFrame, isBlackFrameFromBase64 } from '@/utils/isBlackFrame';
 *
 * // URL 异步用法
 * const result = await isBlackFrame('https://example.com/image.jpg');
 *
 * // Base64 异步用法
 * const result2 = await isBlackFrameFromBase64('data:image/jpeg;base64,...');
 *
 * // 自定义配置
 * const options = {
 *   brightnessThreshold: 40,    // 提高亮度阈值
 *   darkPixelRatio: 0.95,       // 设置暗像素占比要求
 *   sampleStep: 4,              // 增加采样步长，提高性能
 *   avgBrightnessThreshold: 25, // 提高平均亮度阈值
 *   centerWeight: 0.7           // 增加中心区域权重
 * };
 * const result3 = await isBlackFrameFromBase64('data:image/jpeg;base64,...', options);
 */
