...
import { PlayerPlugin } from "./plugins";
...
export class Player extends DPlayer {
    ...
    switchVideo(video: DPlayerVideo, danmaku?: DPlayerDanmaku): void {
        (super.switchVideo as (video: DPlayerVideo, danmaku?: DPlayerDanmaku) => void)(video, danmaku);
    }
    on(event: string, handler: (...args: any[]) => void): void {
        (super.on as (event: string, handler: (...args: any[]) => void) => void)(event, handler);
    }
     ...
     */
    registerPlugin<State>(PluginClass: { new(player: Player, state?: State): PlayerPlugin<State>, pluginName: string }, state?: State) {
