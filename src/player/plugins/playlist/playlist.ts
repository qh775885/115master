...
import { iconNext, iconOpenPlaylist, iconPrve } from "../../icons/icons";
...
export class Playlist extends PlayerPlugin<State> {
	...
	private initOpenListButton() {
		...
		const rightIcons = this.player?.template.controller?.querySelector(
			".dplayer-icons-right",
		);
		...
		const handleDocumentClick = (e: MouseEvent) => {
			...
			const list = this.player?.container.querySelector(
	...
	private initChahgePageButton() {
		...
		// 阻止按钮上的点击事件穿透
		prevButton.addEventListener("mousedown", (e: MouseEvent) =>
			e.stopPropagation(),
		);
		nextButton.addEventListener("mousedown", (e: MouseEvent) =>
			e.stopPropagation(),
		);
		...
		// 获取播放按钮元素
		const playButton =
			this.player?.template.controller?.querySelector(".dplayer-play-icon");
	...
	private renderList(playlistItems: Entity.PlaylistItem[]): string {
		return `
            ${playlistItems
							.map((item, index) => {
								const isPlaying =
									item.pc === this.state?.playingVideoInfo?.pickCode;
								return `
	...
	private onButtonClick() {
		...
		setTimeout(() => {
			// 滚动到当前播放的视频
			const playingItem = list?.querySelector(
			...
			if (playingItem) {
				...
				const targetScrollTop =
					itemOffsetTop -
					(list.clientHeight - (playingItem as HTMLElement).offsetHeight) / 2;
	...
	private onListItemClick(e: MouseEvent) {
		...
		if (item) {
			...
			if (
				itemData.pc &&
				itemData.pc !== this.state.playingVideoInfo?.pickCode
			) {
	...
	private updateNavigationButtons() {
		...
		const currentIndex = this.state.playlist.findIndex(
			(item) => item.pc === this.state.playingVideoInfo?.pickCode,
		);
