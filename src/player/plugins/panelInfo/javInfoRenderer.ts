import { JAV_SOURCE, JavInfo } from '../../../utils/jav/jav';
...
export class JavInfoRenderer {
    ...
    private renderTabContent(source: string, index: number, javInfos: JavInfo[]): string {
        return `
        ...
        const items = [
            {
                ...
                render: () => this.renderContentItem('', `<a href="${info.url}" target="_blank" rel="noopener noreferrer">${info.title}</a>`)
            },
            {
            ...
            },
            {
                ...
                render: () => this.renderContentItem('演员：', info.actors?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(','))
            },
            {
                condition: info.director?.length,
                render: () => this.renderContentItem('导演：', info.director?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(','))
            },
            {
                condition: info.date,
                render: () => this.renderContentItem('日期：', dayjs(info.date).format('YYYY-MM-DD'))
            },
            {
            ...
            },
            {
                ...
                render: () => this.renderContentItem('片商：', info.studio?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(iconSeparator))
            },
            {
                condition: info.publisher?.length,
                render: () => this.renderContentItem('发行商：', info.publisher?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(iconSeparator))
            },
            {
                condition: info.series?.length,
                render: () => this.renderContentItem('系列：', info.series?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(iconSeparator))
            },
            {
                condition: info.category?.length,
                render: () => this.renderContentItem('类别：', info.category?.map(i =>
                    `<a href="${i.url}" target="_blank" rel="noopener noreferrer">${i.name}</a>`).join(iconSeparator))
            },
            {
    ...
    private renderContentItem(label: string, value: string | number | undefined): string {
        return `
