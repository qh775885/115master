        ...
        return `
            <div class="dplayer-panel-title-wrapper">
                ...
                <div class="dplayer-panel-basic-info">
                    ${filePath.map(item => `<a href="${this.getFilePathUrl(item)}" target="_blank">${item.name}</a>`).join(' / ')}
                </div>
                <div class="dplayer-panel-basic-info">
                    ${this.formatFileSize(info.size)} · ${this.formatDate(info.createTime)}
                </div>
            </div>
        `;
    }
